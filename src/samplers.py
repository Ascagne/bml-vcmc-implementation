...
def gibbs_sampler(X, y, nsim=10000, burnin=2000, sigma=1):
    ...
    for i in range(nsim):
        mu_z = np.dot(X, theta)
        z[y == 0] = truncnorm.rvs(-np.inf, -mu_z[y == 0],
                                  loc=mu_z[y == 0], scale=1)
        z[y == 1] = truncnorm.rvs(-mu_z[y == 1], np.inf,
                                  loc=mu_z[y == 1], scale=1)
...
def cmc(X, y, ncores=10, nsim=5000, burnin=1000):
    ...
    for i in range(ncores):
        sub_thetas[i, :, :] = gibbs_sampler(
            X[i*shard:(i+1)*shard, :], y[i*shard:(i+1)*shard], nsim, burnin)
    return sub_thetas
